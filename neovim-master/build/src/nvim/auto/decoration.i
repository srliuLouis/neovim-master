# 1 "/Users/shi-rongliu/tool/neovim-master/src/nvim/decoration.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/Users/shi-rongliu/tool/neovim-master/src/nvim/decoration.c" 2
...
# 5 "/Users/shi-rongliu/tool/neovim-master/src/nvim/decoration.c" 2
...
# 6 "/Users/shi-rongliu/tool/neovim-master/src/nvim/decoration.c" 2
...
# 7 "/Users/shi-rongliu/tool/neovim-master/src/nvim/decoration.c" 2
...
# 8 "/Users/shi-rongliu/tool/neovim-master/src/nvim/decoration.c" 2
...
# 9 "/Users/shi-rongliu/tool/neovim-master/src/nvim/decoration.c" 2
...
# 10 "/Users/shi-rongliu/tool/neovim-master/src/nvim/decoration.c" 2
...
# 11 "/Users/shi-rongliu/tool/neovim-master/src/nvim/decoration.c" 2
...
# 36 "/Users/shi-rongliu/tool/neovim-master/src/nvim/decoration.c"
                             ...
                             colnr_T offset)
{
  ...
  for (linenr_T lnum = pos_start.lnum; lnum <= pos_end.lnum; lnum ++) {
    ...
    if (pos_start.lnum < lnum && lnum < pos_end.lnum) {
      ...
      hl_start = (((offset-1)>(0))?(offset-1):(0));
      end_off = 1;
      hl_end = 0;
    ...
    (void)extmark_set(buf, (uint64_t)src_id, 0,
                      ...
                      decor, 1, 0, kExtmarkNoUndo);
  }
}

Decoration *decor_hl(int hl_id)
{
  (__builtin_expect(!(hl_id > 0), 0) ? __assert_rtn(__func__, "/Users/shi-rongliu/tool/neovim-master/src/nvim/decoration.c", 79, "hl_id > 0") : (void)0);
...
void clear_virttext(VirtText *text)
{
  ...
  *text = (VirtText){ .size = 0, .capacity = 0, .items = ((void*)0) };
}
...
static void decor_add(DecorState *state, int start_row, int start_col,
                      int end_row, int end_col, Decoration *decor, _Bool owned)
{
...
int decor_redraw_col(buf_T *buf, int col, int virt_col, _Bool hidden,
                     DecorState *state)
{
...
next_mark:
  ...
  for (size_t i = 0; i < ((state->active).size); i++) {
    ...
    if (item.end_row < state->row
      ...
      if (item.start_row < state->row
        ...
        if (item.end_row == state->row && item.end_col > col) {
          state->col_until = (((state->col_until)<(item.end_col-1))?(state->col_until):(item.end_col-1));
        }
      } else {
        if (item.start_row == state->row) {
          state->col_until = (((state->col_until)<(item.start_col-1))?(state->col_until):(item.start_col-1));
        }
      }
    }
    ...
    if ((item.start_row == state->row && item.start_col <= col)
        && ((item.decor.virt_text).size) && item.virt_col == -1) {
...
{
  decor_redraw_col(buf, MAXCOL, MAXCOL, 0, state);
  VirtText text = ((VirtText){ .size = 0, .capacity = 0, .items = ((void*)0) });
  for (size_t i = 0; i < ((state->active).size); i++) {
    DecorRange item = ((state->active).items[(i)]);
    if (!((text).size)
        && item.start_row == state->row && ((item.decor.virt_text).size)
                         ...
                         Decoration *decor)
{
  ...
  decor_add(&decor_state, start_row, start_col, end_row, end_col, decor, 1);
}
...
DecorProvider *get_decor_provider(NS ns_id, _Bool force)
{
  ...
  *item = (DecorProvider) { ns_id, 0, (-2), (-2), (-2), (-2), (-2), (-2), -1 };
