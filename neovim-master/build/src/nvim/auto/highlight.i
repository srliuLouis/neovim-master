# 1 "/Users/shi-rongliu/tool/neovim-master/src/nvim/highlight.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 366 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/Users/shi-rongliu/tool/neovim-master/src/nvim/highlight.c" 2
...
# 7 "/Users/shi-rongliu/tool/neovim-master/src/nvim/highlight.c" 2
...
# 8 "/Users/shi-rongliu/tool/neovim-master/src/nvim/highlight.c" 2
...
# 12 "/Users/shi-rongliu/tool/neovim-master/src/nvim/highlight.c" 2
...
# 13 "/Users/shi-rongliu/tool/neovim-master/src/nvim/highlight.c" 2
...
# 14 "/Users/shi-rongliu/tool/neovim-master/src/nvim/highlight.c" 2
...
# 15 "/Users/shi-rongliu/tool/neovim-master/src/nvim/highlight.c" 2
...
# 18 "/Users/shi-rongliu/tool/neovim-master/src/nvim/highlight.c" 2
...
# 19 "/Users/shi-rongliu/tool/neovim-master/src/nvim/highlight.c" 2





static _Bool hlstate_active = 0;

static struct { size_t size; size_t capacity; HlEntry *items; } attr_entries = { .size = 0, .capacity = 0, .items = ((void*)0) };
...
static int get_attr_entry(HlEntry entry)
{
  ...
  if (next_id > 2147483647) {
    logmsg(4, ((void*)0), __func__, 101, 1, "The index on attr_entries has overflowed");
    return 0;
  }
  id = (int)next_id;
...
{
  DecorProvider *p = get_decor_provider(ns_id, 1);
...
int ns_get_hl(NS ns_id, int hl_id, _Bool link, _Bool nodefault)
{
  ...
  _Bool valid_cache = it.version >= p->hl_valid;
  ...
  if (!valid_cache && p->hl_def != (-2) && !recursive) {
    ...
    recursive++;
    Object ret = nlua_call_ref(p->hl_def, "hl_def", args, 1, &err);
    recursive--;
    ...
    if (ret.type == kObjectTypeDictionary) {
      ...
      fallback = 0;
      attrs = dict2hlattrs(dict, 1, &it.link_id, &err);
      for (size_t i = 0; i < dict.size; i++) {
        char *key = dict.items[i].key.data;
        Object val = dict.items[i].value;
        _Bool truthy = api_object_to_bool(val, key, 0, &err);
...
void update_window_hl(win_T *wp, _Bool invalid)
{
  ...
  _Bool has_blend = wp->w_floating && wp->w_onebuf_opt.wo_winbl != 0;


  _Bool float_win = wp->w_floating && !wp->w_float_config.external;
  ...
  int ns_attr = ns_get_hl(-1, normality, 0, 0);
  if (ns_attr > 0) {
  ...
  for (int hlf = 0; hlf < (int)HLF_COUNT; hlf++) {
    ...
    if (wp->w_hl_ids[hlf] != 0) {
      attr = hl_get_ui_attr(hlf, wp->w_hl_ids[hlf], 0);
    } else {
...
void clear_hl_tables(_Bool reinit)
{
  if (reinit) {
    ...
    __builtin___memset_chk (highlight_attr_last, -1, sizeof(highlight_attr_last), __builtin_object_size (highlight_attr_last, 0));
...
static HlAttrs get_colors_force(int attr)
{
  ...
  do { _Bool dark_ = (*p_bg == 'd'); attrs.rgb_fg_color = attrs.rgb_fg_color != -1 ? attrs.rgb_fg_color : (dark_ ? 0xFFFFFF : 0x000000); attrs.rgb_bg_color = attrs.rgb_bg_color != -1 ? attrs.rgb_bg_color : (dark_ ? 0x000000 : 0xFFFFFF); attrs.rgb_sp_color = attrs.rgb_sp_color != -1 ? attrs.rgb_sp_color : 0xFF0000; } while (0);;
...
{
  Dictionary hl = {.size = 0, .capacity = 0, .items = ((void*)0)};
  int mask = use_rgb ? ae.rgb_ae_attr : ae.cterm_ae_attr;

  if (mask & HL_BOLD) {
    (*((((hl).size == (hl).capacity) ? (((hl).capacity = ((hl).capacity ? (hl).capacity << 1 : 8), (hl).items = xrealloc((hl).items, sizeof((hl).items[0]) * (hl).capacity)), 0) : 0), ((hl).items + ((hl).size++))) = (((KeyValuePair) { .key = cstr_to_string("bold"), .value = ((Object) { .type = kObjectTypeBoolean, .data.boolean = 1 }) })));
  }

  if (mask & HL_STANDOUT) {
    (*((((hl).size == (hl).capacity) ? (((hl).capacity = ((hl).capacity ? (hl).capacity << 1 : 8), (hl).items = xrealloc((hl).items, sizeof((hl).items[0]) * (hl).capacity)), 0) : 0), ((hl).items + ((hl).size++))) = (((KeyValuePair) { .key = cstr_to_string("standout"), .value = ((Object) { .type = kObjectTypeBoolean, .data.boolean = 1 }) })));
  }

  if (mask & HL_UNDERLINE) {
    (*((((hl).size == (hl).capacity) ? (((hl).capacity = ((hl).capacity ? (hl).capacity << 1 : 8), (hl).items = xrealloc((hl).items, sizeof((hl).items[0]) * (hl).capacity)), 0) : 0), ((hl).items + ((hl).size++))) = (((KeyValuePair) { .key = cstr_to_string("underline"), .value = ((Object) { .type = kObjectTypeBoolean, .data.boolean = 1 }) })));
  }

  if (mask & HL_UNDERCURL) {
    (*((((hl).size == (hl).capacity) ? (((hl).capacity = ((hl).capacity ? (hl).capacity << 1 : 8), (hl).items = xrealloc((hl).items, sizeof((hl).items[0]) * (hl).capacity)), 0) : 0), ((hl).items + ((hl).size++))) = (((KeyValuePair) { .key = cstr_to_string("undercurl"), .value = ((Object) { .type = kObjectTypeBoolean, .data.boolean = 1 }) })));
  }

  if (mask & HL_ITALIC) {
    (*((((hl).size == (hl).capacity) ? (((hl).capacity = ((hl).capacity ? (hl).capacity << 1 : 8), (hl).items = xrealloc((hl).items, sizeof((hl).items[0]) * (hl).capacity)), 0) : 0), ((hl).items + ((hl).size++))) = (((KeyValuePair) { .key = cstr_to_string("italic"), .value = ((Object) { .type = kObjectTypeBoolean, .data.boolean = 1 }) })));
  }

  if (mask & HL_INVERSE) {
    (*((((hl).size == (hl).capacity) ? (((hl).capacity = ((hl).capacity ? (hl).capacity << 1 : 8), (hl).items = xrealloc((hl).items, sizeof((hl).items[0]) * (hl).capacity)), 0) : 0), ((hl).items + ((hl).size++))) = (((KeyValuePair) { .key = cstr_to_string("reverse"), .value = ((Object) { .type = kObjectTypeBoolean, .data.boolean = 1 }) })));
  }

  if (mask & HL_STRIKETHROUGH) {
    (*((((hl).size == (hl).capacity) ? (((hl).capacity = ((hl).capacity ? (hl).capacity << 1 : 8), (hl).items = xrealloc((hl).items, sizeof((hl).items[0]) * (hl).capacity)), 0) : 0), ((hl).items + ((hl).size++))) = (((KeyValuePair) { .key = cstr_to_string("strikethrough"), .value = ((Object) { .type = kObjectTypeBoolean, .data.boolean = 1 }) })));
  }

  if (use_rgb) {
    if (mask & HL_FG_INDEXED) {
      (*((((hl).size == (hl).capacity) ? (((hl).capacity = ((hl).capacity ? (hl).capacity << 1 : 8), (hl).items = xrealloc((hl).items, sizeof((hl).items[0]) * (hl).capacity)), 0) : 0), ((hl).items + ((hl).size++))) = (((KeyValuePair) { .key = cstr_to_string("fg_indexed"), .value = ((Object) { .type = kObjectTypeBoolean, .data.boolean = 1 }) })));
    }

    if (mask & HL_BG_INDEXED) {
      (*((((hl).size == (hl).capacity) ? (((hl).capacity = ((hl).capacity ? (hl).capacity << 1 : 8), (hl).items = xrealloc((hl).items, sizeof((hl).items[0]) * (hl).capacity)), 0) : 0), ((hl).items + ((hl).size++))) = (((KeyValuePair) { .key = cstr_to_string("bg_indexed"), .value = ((Object) { .type = kObjectTypeBoolean, .data.boolean = 1 }) })));
    }
  ...
  for (size_t i = 0; i < dict.size; i++) {
    ...
    for (j = 0; flags[j].name; j++) {
      if (strequal(flags[j].name, key)) {
        if (api_object_to_bool(val, key, 0, err)) {
...
{
  Dictionary item = {.size = 0, .capacity = 0, .items = ((void*)0)};
  if (attr <= 0 || attr >= (int)((attr_entries).size)) {
    return;
  }
